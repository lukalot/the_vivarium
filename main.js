...
class ASCIITerminal {
  ...
  updateTerminalDimensions() {
    ...
    const clampedSize = Math.max(
      this.minCharSize,
      Math.min(this.maxCharSize, baseSize),
    );
    ...
    if (terminalSizeEl)
      terminalSizeEl.textContent = `${this.cols}×${this.rows}`;
    if (targetSizeEl)
      targetSizeEl.textContent = `${this.targetCols}×${this.targetRows}`;
    if (windowSizeEl)
      windowSizeEl.textContent = `${window.innerWidth}×${window.innerHeight}`;
    if (charSizeEl)
      charSizeEl.textContent = `${this.charWidth}×${this.charHeight}`;
    if (scaleFactorEl)
      scaleFactorEl.textContent = `${(scaleFactor * 100).toFixed(1)}%`;
    if (pixelRatioEl) pixelRatioEl.textContent = `${window.devicePixelRatio}x`;

    console.log(
      `Updated terminal: ${this.cols}×${this.rows} chars (${(scaleFactor * 100).toFixed(1)}% of target), size: ${this.charWidth}×${this.charHeight}px, DPR: ${window.devicePixelRatio}x`,
    );
  }
  ...
  createCharacterMeshes() {
    ...
    const rowBackgroundGeometry = new THREE.PlaneGeometry(
      rowBackgroundWidth,
      this.charHeight,
    );
  ...
  updateCharacter(x, y, char, customColor = null) {
    ...
    if (x >= 0 && x < this.cols && y >= 0 && y < this.rows) {
      ...
      if (mesh) {
        // Determine if this should be a rainbow character
        const posKey = `${x},${y}`;
        const isRainbow = this.isRainbowCharacter.get(posKey);

        // Calculate color for this character
        let textColor = customColor || "#ffffff"; // Default to white
        ...
        // Create cache key - use color index for rainbow to improve performance
        const cacheKey = isRainbow ? `${char}_rainbow_${colorIndex}` : char;
  ...
  scrollUp() {
    ...
    for (let y = 0; y < this.rows - 1; y++) {
      for (let x = 0; x < this.cols; x++) {
        ...
        const char =
          this.buffer[sourceScreenY] &&
          this.buffer[sourceScreenY][sourceScreenX]
            ? this.buffer[sourceScreenY][sourceScreenX]
            : " ";
  ...
  writeCenteredText(text, isRainbow = false) {
    ...
    for (const line of lines) {
      ...
      // Mark characters as rainbow if requested
      if (isRainbow) {
        for (let i = 0; i < line.length; i++) {
          const posKey = `${centerX + i},${this.cursor.y}`;
          this.isRainbowCharacter.set(posKey, true);
        }
      }
    ...
    // Update title area bounds for rainbow calculation
    if (isRainbow) {
      const endY = this.cursor.y - 1;
      const maxLineLength = Math.max(...lines.map((l) => l.length));
      this.titleArea = {
        startY: startY,
        endY: endY,
        centerX: Math.floor(this.cols / 2),
  ...
  async demoText(titleFile = "glowy.txt") {
    ...
    // Load and display the ASCII art title (centered) with rainbow animation
    const titleArt = await this.loadTitleFromFile(titleFile);
    this.writeCenteredText(titleArt, true); // Enable rainbow for title
    ...
    this.writeCenteredText(randomMessage, true); // Enable rainbow for subtitle too
  ...
  redrawInputLine() {
    ...
    for (let i = this.inputStartX; i < this.cols; i++) {
      ...
      if (
        this.characterMeshes[screenY] &&
        this.characterMeshes[screenY][screenX]
      ) {
  ...
  animate() {
    ...
    // Update rainbow animation frame counter
    this.rainbowFrame++;

    // Only update rainbow characters every N frames for performance
    if (
      this.isRainbowCharacter.size > 0 &&
      this.rainbowFrame % this.rainbowUpdateInterval === 0
    ) {
      // Update all rainbow characters
      this.isRainbowCharacter.forEach((isRainbow, posKey) => {
        if (isRainbow) {
          const [x, y] = posKey.split(",").map(Number);
          const screenX = x + this.paddingX;
          const screenY = y + this.paddingY;
          const char =
            this.buffer[screenY] && this.buffer[screenY][screenX]
              ? this.buffer[screenY][screenX]
              : " ";
          if (char !== " ") {
            this.updateCharacter(x, y, char);
          }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  console.log(
    "Row backgrounds: Even=#0a0a0a, Odd=#121212 (use useVisibleRowColors() for better visibility)",
  );
  console.log(
    "Try useVisibleRowColors() for more visible background alternation!",
  );
  ...
  window.testPadding = () => {
    ...
    console.log(
      `Padding test: Text area is ${terminal.cols}x${terminal.rows}, should be offset by ${terminal.paddingX} chars horizontally and ${terminal.paddingY} chars vertically from screen edges`,
    );
  };

  // Add some helpful console functions for experimenting with scaling
  window.setTerminalTarget = (cols, rows) =>
    terminal.setTargetDimensions(cols, rows);
  window.setCharSizeLimits = (min, max) =>
    terminal.setCharacterSizeLimits(min, max);
  window.setPadding = (paddingX, paddingY) =>
    terminal.setPadding(paddingX, paddingY);
  window.setTitleVerticalPadding = (lines) =>
    terminal.setTitleVerticalPadding(lines);
  ...
  window.testChar = (char = "A", x = 5, y = 5) => {
    ...
    console.log(
      `Test: Drew "${char}" (code: ${char.charCodeAt(0)}) at position ${x},${y}`,
    );
  };
  ...
  window.showDebugPanel = () => {
    ...
    document
      ...
      .insertAdjacentHTML("beforeend", debugHTML);
  ...
  window.testColors = () => {
    ...
    terminal.inputBuffer = "";
    console.log(
      "Full-width background test displayed - backgrounds span entire screen width",
    );
  };
  ...
  window.setRowColors = (evenBgColor, oddBgColor) => {
    ...
    terminal.updateRowBackgrounds();
    console.log(
      `Row background colors updated: Even=${evenBgColor}, Odd=${oddBgColor}`,
    );
  };
  ...
  window.testVisibleColors = () => {
    ...
    console.log(
      '1. setRowColors("#2a2a2a", "#1a1a1a") - Subtle gray alternation',
    );
    console.log(
      '2. setRowColors("#1a1a2a", "#0a0a1a") - Subtle blue alternation',
    );
    console.log(
      '3. setRowColors("#2a1a1a", "#1a0a0a") - Subtle red alternation',
    );
    console.log(
      '4. setRowColors("#1a2a1a", "#0a1a0a") - Subtle green alternation',
    );
    console.log(
      '5. setRowColors("#2a1a2a", "#1a0a1a") - Subtle purple alternation',
    );
  ...
  window.resetRowColors = (singleBgColor = "#000000") => {
    ...
    terminal.updateRowBackgrounds();
    console.log(
      `Row background colors reset to single color: ${singleBgColor}`,
    );
  };
  ...
  window.useVisibleRowColors = () => {
    ...
    console.log(
      "Applied more visible row background colors: #1a1a1a / #2a2a2a",
    );
  };
  ...
  window.testTyping = () => {
    ...
    console.log(
      `Current cursor position: ${terminal.cursor.x}, ${terminal.cursor.y}`,
    );
  };
  ...
  terminal.handleEnter = function () {
    ...
    if (
      window.onTerminalInput &&
      typeof window.onTerminalInput === "function"
    ) {
      try {
